...
9. ### Categorical features
   ...
   2. Draw bar chart for value_counts()
      ...
      1. ```
         ...
         import seaborn as sns
         b = sns.countplot(x='col_name', data=df)
         b.set_xlabel('col_name', fontsize=20)
   ...
   3. df.groupby
      ...
      1. ```
         # How many 'id's do we have in the df where the value of ord_2 is 'Boiling Hot'?
         ...
         ```
   4. New numerical feature from categorical feature groupby count

      1. ```
         df['col_name_groupby'] = df.groupby(['col_name'])['id'].transform('count')
         ```
   5. New categorical feature from combining other categorical features

      1. ```
         df['new_feature'] = (df.col1_name.astype(str)) + '_' + (df.col2_name.astype(str))
         ```
   6. Fill NaN for categorical features

      1. ```
         df.loc[:, 'col_name'] = df.col_name.fillna('NONE')    # Treat the NaN as a new category "NONE"
         ```
   7. TODO
   8. TODO
   9. Convert by LabelEncoder

      1. ```
         import pandas as pd
         from sklearn import preprocessing

         df.loc[:, 'ord_2'] = df.ord_2.fillna("NONE")    # LabelEncoder doesn't support NaN

         enc = preprocessing.LabelEncoder()

         df.loc[:, 'ord_2'] = enc.fit_transform(df.ord_2.values)
         ```
   10. Convert categorical feature into Integers by OneHotEncode
       ...
       1. ```
          ...
          ct.fit_transform(df)
          ```
   11. Unknown categorical features

       1. ```
          import pandas as pd
          from sklearn import preprocessing

          def combine_train_test_categorical_features(train_csv, test_csv):
              ...
              for feat in features:
                  ...
                  # Note the trick here
                  # Since it's categorical data, we fillna with a string
                  # So we can convert all the data to string type 
                  # This means that, no matter it's int or float, it's converted to string
                  ...
                  data.loc[:, feat] = le.fit_transform(temp_col)

              # split the training and test data again
              train = data[data.target != -1].reset_index(drop=True)
              test = data[data.target == -1].reset_index(drop=True)
          ```
   12. Rare categorical features

       1. ```
          df.ord_4 = df.ord_4.fillna('NONE')

          df.loc[
              df['ord_4'].value_counts()[df['ord_4']].values < 1,     # Define rare, if it appears only 1 time for column='ord_4'
              'ord_4'
              ] = 'RARE'
          ```
   13. TODO
   14. TODO
10. Categorical Feature steps

    1. Fill NaN
    2. Rare Category:
       1. use 'unknown'
       2. Train a simple model to predict the feature when it's unknown
    3. TODO
11. ### Numerical + Categorical features Preprocessing Pipeline
    ...
    ```
12. ### Preprocessor Save & Load in Deployment
